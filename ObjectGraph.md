...
---
# Object Graph
An interactive object-graph explorer for SilverBullet. Anchored on the current page, it visualizes objects (pages, items, blocks, …) and the typed relations between them — letting you navigate your space by structure rather than by name.
...
* **Double-click any node** to navigate to it (pages, items, and blocks open in the editor; URLs open externally).
* **Double-click an edge** to navigate to the source of that relation (the page + position where the link / attribute lives).
* **Hover an edge** to see its label and snippet.
* **Drag** a node to pin it; the layout reflows around it.
* **Hover** dims everything outside the node's neighborhood.
...
* **Left — Sidebar.**
  * **Root tags** — toggle visibility by structural object kind (`page`, `item`, `task`, `block`, …).
  * **Tags** — toggle visibility by user tag. Untagged objects share an `(untagged)` bucket.
  * **Relations** — toggle typed edge labels (`mention`, `co-mention`, custom relation types, …). `co-mention` is hidden by default in Explore mode; only `mention` is visible by default in Global Page Map mode.
...
* **Dashed outline** — dangling refs (referenced by some relation but not yet indexed).
* **Color** is driven by the node's primary user tag, auto-derived from the tag name. Untagged nodes use a neutral gray.
* **Edge labels** are typed relation names (or the raw kind like `mention` when no type is set), drawn along the edge. Directed edges carry an arrowhead at the target; relations that connect the same pair in both directions get arrows at both ends; collapsed `co-mention` pairs are rendered undirected.
