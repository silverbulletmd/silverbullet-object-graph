...
export class GraphCanvas extends Component<Props, State> {
	...
	private fg: ReturnType<typeof ForceGraph> | null = null;
	...
	private async handleDoubleClick(node: FGNode) {
		try {
			if (node.kind === "url") {
				...
				// page / item / block / file — editor.navigate accepts a ref string.
				await editor.navigate(node.id);
				await editor.hidePanel("modal");
			}
		} catch (err) {
	...
	private drawLink(link: FGLink, ctx: CanvasRenderingContext2D, scale: number) {
		...
		const kind: RelationKind = link.edge.kind;
		const dimmed = kind === "co-mention" || kind === "mention";
		const hov = this.hoveredId;
		...
		if (showLabel) {
			...
			ctx.fillStyle = link.edge.type ? t.label : t.labelDim;
